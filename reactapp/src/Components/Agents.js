...
const Agents = () => {
  ...
  useEffect(()=>{
    axios.get('http://localhost:8080/api/v1/employees').then(res=>setData(res.data)).catch(err=>console.log(err));
  },[])
  const deleteData=(id,e)=>{
    axios.delete(`http://localhost:8080/api/v1/employees/${id}`).then(()=>{if(window.confirm('Deleted')){window.location.reload();}});
  }
        ...
        <Card style={{backgroundColor:"#ff0000",height:"175px",width:"200px"}}>
          <Card.Body>
            ...
            <Link to="/AdminHomepage/AgentRecord" style={{textDecoration:"none"}}>
              <h4 className='first'style={{color:"black"}}>Agent Record</h4>
            </Link>
          </Card.Body>
        </Card>
      </Col>
        ...
        <Card style={{backgroundColor:"#FC8213",height:"175px",width:"200px"}}>
          <Card.Body>
            ...
            <Link to="/AdminHomepage/AddAgent" style={{textDecoration:"none"}} >
              <h4 className='second' style={{color:"black"}}>Add Agent</h4>
            </Link>
          </Card.Body>
        </Card>
      </Col>
    ...
    <table class="table table-striped" id="dev-table" style={{border:'1px solid'}}>
      ...
      {data.map((item)=>{
        ...
        <tr>
          ...
          <td>{item.lastname}</td>
          <td>
            <Button variant='primary' onClick={()=>navigate("/UpdateAgent",{ state: { id: item.id} })}>
            <AiIcons.AiFillEdit/>
            </Button>
          </td>
          <td>
          <Button variant='primary' onClick={(e) => deleteData(item.id,e)}>
            <FaIcons.FaTrash/>
          </Button>
          </td>
        </tr>
